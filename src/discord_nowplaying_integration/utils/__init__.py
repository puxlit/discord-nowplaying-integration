...
def format_status_message(artist, title):
    ...
    'Beyond the Clouds (by audiomachine)'
    ...
    ... )
    'Ellen\'s Gesang III (Ave Maria!), Op. 56, No. 6, D. 839, "Hymne an Die Jungfrau" (by Daniel Winiger, Daniel Perret & […])'
    ...
    ... )
    'Symphonie No. 6 in A-Dur, Op. 68 - "Pastorale": Erwachen heiterer Gefühle bei der Ankunft […] (by Gustav Kuhn, Haydn […])'
    ...
    max_bytes = 128
    max_bytes_available = max_bytes - len(separator.encode()) - len(suffix.encode())
    ...
    status_message = title + separator + artist + suffix
